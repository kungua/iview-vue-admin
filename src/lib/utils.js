...
export const setTitle = title => {
  window.document.title = title || '后台'
}
...
export const downloadFiles = ({ url, params }) => {
...
export const getTabNameByRoute = ({ name, params, query }) => {
  let res = name
...
const getObjectBySplitStr = (id, splitStr) => {
...
export const getRouteById = id => {
  let res = {}
  if (id.includes('&')) {
    res.query = getObjectBySplitStr(id, '&')
    id = id.split('&')[0]
  }
  if (id.includes(':')) {
    res.params = getObjectBySplitStr(id, ':')
    id = id.split(':')[0]
  }
  res.name = id
  return res
